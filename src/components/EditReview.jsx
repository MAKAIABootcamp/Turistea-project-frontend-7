...
const EditReview = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      ecology: Yup.string().required(
        "Seleccionar si es ecológico es obligatorio"
      ),
      lowCost: Yup.string().required(
        "Seleccionar si es de bajo costo es obligatorio"
      ),
            ...
            {Array.from({ length: 3 }).map((_, index) => (
              ...
              >
                ...
                {secondaryImages[index] ? (
                  <img
                    src={
                      typeof secondaryImages[index] === "string"
                        ? secondaryImages[index]
                        : URL.createObjectURL(secondaryImages[index])
                    }
                    alt={`Secondary ${index}`}
                    className="w-full h-full object-cover"
                  />
